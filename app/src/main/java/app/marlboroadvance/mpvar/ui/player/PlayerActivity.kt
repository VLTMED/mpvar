...
import app.marlboroadvance.mpvar.databinding.PlayerLayoutBinding
...
import app.marlboroadvance.mpvar.utils.storage.FileTypeUtils
import app.marlboroadvance.mpvar.utils.storage.FileFilterUtils
...
class PlayerActivity :
   ...
   */
  private val playlistRepository: app.marlboroadvance.mpvar.database.repository.PlaylistRepository by inject()
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        val siblingFiles = if (launchSource == "video_list" || launchSource == "recently_played_button" || launchSource == "first_video_button") {
          ...
          val videosInFolder =
            app.marlboroadvance.mpvar.repository.MediaFileRepository.getVideosForBuckets(
              context,
              setOf(bucketId)
            )
          val sortedVideos = app.marlboroadvance.mpvar.utils.sort.SortUtils.sortVideos(videosInFolder, videoSortType, videoSortOrder)
          sortedVideos.mapNotNull { video -> files.find { it.absolutePath == video.path } }
        } else {
          files.sortedWith { f1, f2 -> app.marlboroadvance.mpvar.utils.sort.SortUtils.NaturalOrderComparator.DEFAULT.compare(f1.name, f2.name) }
        }
  ...
  companion object {
     ...
     */
    private const val RESULT_INTENT = "app.marlboroadvance.mpvar.ui.player.PlayerActivity.result"
