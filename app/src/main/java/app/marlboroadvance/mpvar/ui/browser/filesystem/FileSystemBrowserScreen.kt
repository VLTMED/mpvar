...
import app.marlboroadvance.mpvar.BuildConfig
...
import app.marlboroadvance.mpvar.preferences.MediaLayoutMode
import app.marlboroadvance.mpvar.preferences.preference.collectAsState
import app.marlboroadvance.mpvar.presentation.components.pullrefresh.PullRefreshBox
import app.marlboroadvance.mpvar.ui.browser.cards.FolderCard
import app.marlboroadvance.mpvar.ui.browser.cards.VideoCard
import app.marlboroadvance.mpvar.ui.browser.components.BrowserBottomBar
import app.marlboroadvance.mpvar.ui.browser.components.BrowserTopBar
import app.marlboroadvance.mpvar.ui.browser.dialogs.AddToPlaylistDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.DeleteConfirmationDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.FileOperationProgressDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.FolderPickerDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.RenameDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.SortDialog
import app.marlboroadvance.mpvar.ui.browser.dialogs.ViewModeSelector
import app.marlboroadvance.mpvar.ui.browser.dialogs.VisibilityToggle
import app.marlboroadvance.mpvar.ui.browser.selection.rememberSelectionManager
import app.marlboroadvance.mpvar.ui.browser.sheets.PlayLinkSheet
import app.marlboroadvance.mpvar.ui.browser.states.EmptyState
import app.marlboroadvance.mpvar.ui.browser.states.PermissionDeniedState
import app.marlboroadvance.mpvar.ui.utils.LocalBackStack
import app.marlboroadvance.mpvar.utils.media.CopyPasteOps
import app.marlboroadvance.mpvar.utils.media.MediaUtils
 ...
 */
@Serializable
object FileSystemBrowserRootScreen : app.marlboroadvance.mpvar.presentation.Screen {
...
data class FileSystemDirectoryScreen(
  val path: String,
) : app.marlboroadvance.mpvar.presentation.Screen {
  @OptIn(ExperimentalPermissionsApi::class)
  @Composable
  override fun Content() {
...
fun FileSystemBrowserScreen(path: String? = null) {
  ...
  val playerPreferences = koinInject<app.marlboroadvance.mpvar.preferences.PlayerPreferences>()
  ...
  val navigationBarHeight = app.marlboroadvance.mpvar.ui.browser.LocalNavigationBarHeight.current
  ...
  LaunchedEffect(
    ...
    if (isAtRoot) {
      try {
        val mainScreenObj = app.marlboroadvance.mpvar.ui.browser.MainScreen
          ...
          val results = if (isAtRoot) {
            ...
            parentDirectories.forEach { parentPath ->
              try {
                ...
                val parentResults = app.marlboroadvance.mpvar.ui.browser.filesystem.searchRecursively(context, parentPath, searchQuery)
            ...
            items.filterIsInstance<FileSystemItem.Folder>().forEach { storageVolume ->
              try {
                ...
                val rootResults = app.marlboroadvance.mpvar.ui.browser.filesystem.searchRecursively(context, storageVolume.path, searchQuery)
            ...
            val results = app.marlboroadvance.mpvar.ui.browser.filesystem.searchRecursively(context, currentPath, searchQuery)
  ...
  app.marlboroadvance.mpvar.ui.browser.fab.FabScrollHelper.trackScrollForFabVisibility(
    listState = listState,
    gridState = null,
    ...
    Scaffold(
      topBar = {
        if (isSearching) {
          ...
          BrowserTopBar(
            title = if (isAtRoot) {
              stringResource(app.marlboroadvance.mpvar.R.string.app_name)
            } else {
            ...
            onSettingsClick = {
              backstack.add(app.marlboroadvance.mpvar.ui.preferences.PreferencesScreen)
            },
            ...
            onInfoClick = if (videoSelectionManager.isInSelectionMode && !folderSelectionManager.isInSelectionMode) {
              {
                ...
                if (video != null) {
                  val intent = Intent(context, app.marlboroadvance.mpvar.ui.mediainfo.MediaInfoActivity::class.java)
          ...
          FloatingActionButtonMenu(
                ...
                ToggleFloatingActionButton(
                  modifier = Modifier
                    .animateFloatingActionButton(
                      visible = !isInSelectionMode && isFabVisible.value && !app.marlboroadvance.mpvar.ui.browser.MainScreen.getPermissionDeniedState(),
            ...
            FloatingActionButtonMenuItem(
              onClick = {
                isFabExpanded.value = false
                coroutineScope.launch {
                  val recentlyPlayedVideos = app.marlboroadvance.mpvar.utils.history.RecentlyPlayedOps.getRecentlyPlayed(limit = 1)
                    ...
                    if (playlistMode) {
                      ...
                      if (startIndex >= 0) {
                        if (allVideos.size == 1) {
                          ...
                          intent.setClass(context, app.marlboroadvance.mpvar.ui.player.PlayerActivity::class.java)
...
suspend fun searchRecursively(
  ...
  try {
    ...
    val items = app.marlboroadvance.mpvar.repository.MediaFileRepository
...
private suspend fun collectVideosRecursively(
  context: Context,
  folderPath: String,
): List<app.marlboroadvance.mpvar.domain.media.model.Video> {
  val videos = mutableListOf<app.marlboroadvance.mpvar.domain.media.model.Video>()
...
private fun playVideosAsPlaylist(
  context: Context,
  videos: List<app.marlboroadvance.mpvar.domain.media.model.Video>,
) {
...
private fun FileSystemBrowserContent(
  ...
  error: String?,
  isAtRoot: Boolean,
  breadcrumbs: List<app.marlboroadvance.mpvar.domain.browser.PathComponent>,
  ...
  onVideoClick: (app.marlboroadvance.mpvar.domain.media.model.Video) -> Unit,
  onVideoLongClick: (app.marlboroadvance.mpvar.domain.media.model.Video) -> Unit,
  onBreadcrumbClick: (app.marlboroadvance.mpvar.domain.browser.PathComponent) -> Unit,
  folderSelectionManager: app.marlboroadvance.mpvar.ui.browser.selection.SelectionManager<FileSystemItem.Folder, String>,
  videoSelectionManager: app.marlboroadvance.mpvar.ui.browser.selection.SelectionManager<app.marlboroadvance.mpvar.domain.media.model.Video, Long>,
  modifier: Modifier = Modifier,
  isInSelectionMode: Boolean = false,
) {
  val gesturePreferences = koinInject<GesturePreferences>()
  val browserPreferences = koinInject<BrowserPreferences>()
  val thumbnailRepository = koinInject<app.marlboroadvance.mpvar.domain.thumbnail.ThumbnailRepository>()
        ...
        Box(
          ...
          LazyColumn(
            ...
            if (!isAtRoot && breadcrumbs.isNotEmpty()) {
              item {
                app.marlboroadvance.mpvar.ui.browser.filesystem.BreadcrumbNavigation(
                  breadcrumbs = breadcrumbs,
            ...
            items(
              ...
              val folderModel = app.marlboroadvance.mpvar.domain.media.model.VideoFolder(
...
fun FileSystemSortDialog(
  ...
  val browserPreferences = koinInject<BrowserPreferences>()
  val appearancePreferences = koinInject<app.marlboroadvance.mpvar.preferences.AppearancePreferences>()
  ...
  SortDialog(
    ...
    onSortTypeChange = { typeName ->
      app.marlboroadvance.mpvar.preferences.FolderSortType.entries.find { it.displayName == typeName }?.let {
    ...
    onSortOrderChange = { isAsc ->
      browserPreferences.folderSortOrder.set(
        if (isAsc) app.marlboroadvance.mpvar.preferences.SortOrder.Ascending
        else app.marlboroadvance.mpvar.preferences.SortOrder.Descending,
      )
    },
    types = listOf(
      app.marlboroadvance.mpvar.preferences.FolderSortType.Title.displayName,
      app.marlboroadvance.mpvar.preferences.FolderSortType.Date.displayName,
      app.marlboroadvance.mpvar.preferences.FolderSortType.Size.displayName,
    ),
    icons = listOf(
      Icons.Filled.Title,
    ...
    getLabelForType = { type, _ ->
      when (type) {
        app.marlboroadvance.mpvar.preferences.FolderSortType.Title.displayName -> Pair("أ-ي", "ي-أ")
        app.marlboroadvance.mpvar.preferences.FolderSortType.Date.displayName -> Pair("الأقدم", "الأحدث")
        app.marlboroadvance.mpvar.preferences.FolderSortType.Size.displayName -> Pair("الأصغر", "الأكبر")
    ...
    viewModeSelector = ViewModeSelector(
      ...
      isFirstOptionSelected = folderViewMode == app.marlboroadvance.mpvar.preferences.FolderViewMode.AlbumView,
      onViewModeChange = { isFirstOption ->
        browserPreferences.folderViewMode.set(
          if (isFirstOption) {
            app.marlboroadvance.mpvar.preferences.FolderViewMode.AlbumView
          } else {
            app.marlboroadvance.mpvar.preferences.FolderViewMode.FileManager
          },
        )
      },
    ),
