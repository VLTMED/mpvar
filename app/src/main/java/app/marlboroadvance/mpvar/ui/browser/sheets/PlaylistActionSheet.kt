package app.marlboroadvance.mpvar.ui.browser.sheets
...
fun PlaylistActionSheet(
  isOpen: Boolean,
  onDismiss: () -> Unit,
  repository: app.marlboroadvance.mpvar.database.repository.PlaylistRepository,
    ...
    Column(
      ...
      Card(
        ...
        Row(
          ...
          Column(modifier = Modifier.weight(1f)) {
            Text(
              text = "إنشاء قائمة تشغيل فارغة",
              style = MaterialTheme.typography.bodyLarge,
              fontWeight = FontWeight.Medium,
            )
            Text(
              text = "إنشاء قائمة تشغيل جديدة فارغة",
      ...
      Card(
        ...
        Row(
          ...
          Column(modifier = Modifier.weight(1f)) {
            Text(
              text = "إضافة قائمة تشغيل M3U من URL",
              style = MaterialTheme.typography.bodyLarge,
              fontWeight = FontWeight.Medium,
            )
            Text(
              text = "استيراد قائمة تشغيل من عنوان URL على الويب",
              style = MaterialTheme.typography.bodySmall,
              color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
          }
                ...
                if (playlistName.isNotBlank()) {
                  coroutineScope.launch {
                    try {
                      ...
                      android.widget.Toast.makeText(
                        context,
                        "فشل إنشاء قائمة التشغيل: ${e.message}",
      ...
      uri?.let {
        ...
        coroutineScope.launch {
          ...
          result.onSuccess {
            android.widget.Toast.makeText(
              context,
              "تمت إضافة قائمة تشغيل M3U بنجاح",
              android.widget.Toast.LENGTH_SHORT
            ).show()
          }.onFailure { error ->
            android.widget.Toast.makeText(
              context,
              "فشل في إضافة قائمة تشغيل M3U: ${error.message}",
              android.widget.Toast.LENGTH_LONG
            ).show()
          }
          isLoading = false
        ...
        Column(
          ...
          Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(
              ...
              label = { Text("عنوان URL لقائمة التشغيل") },
              singleLine = false,
              maxLines = 3,
              modifier = Modifier.fillMaxWidth(),
              enabled = !isLoading
