package app.marlboroadvance.mpvar.utils.history

import android.annotation.SuppressLint
import android.net.Uri
import app.marlboroadvance.mpvar.database.entities.RecentlyPlayedEntity
import app.marlboroadvance.mpvar.domain.recentlyplayed.repository.RecentlyPlayedRepository
...
object RecentlyPlayedOps {
  ...
  suspend fun getLastPlayedEntity(): app.marlboroadvance.mpvar.database.entities.RecentlyPlayedEntity? {
    return withContext(Dispatchers.IO) {
      val recent = kotlin.runCatching { repository.getRecentlyPlayed(limit = 50) }.getOrDefault(emptyList())
      for (entity in recent) {
        val path = entity.filePath
