/**
 * @file header.tsx
 * @description رأس التطبيق مع تصميم RTL أصيل ودعم عربي كامل
...
export function Header({ downloadUrl }: { downloadUrl?: string }) {
  ...
  const navLinks = [
    ...
    { name: "المساهمون", href: "#contributors" },
  ];

  return (
    <header
      className="fixed top-4 z-50 md:top-6"
      style={{
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            className="text-foreground text-lg tracking-tight group-hover:text-primary transition-colors"
        ...
        {/* التنقل المركزي للشاشات الكبيرة */}
        ...
        {/* أزرار الإجراءات - يسار في RTL */}
        <div className="hidden md:flex items-center gap-3">
          {/* زر التحميل - الإجراء الرئيسي في أقصى اليسار */}
          <Button
          ...
          >
            ...
            <span className="hidden sm:inline">نجمة</span>
          </button>
        </div>
        ...
        <div className="lg:hidden flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 w-10 h-10 flex items-center justify-center bg-foreground/5 hover:bg-foreground/10 rounded-2xl transition-all active:scale-95"
      ...
      {/* قائمة الجوال */}
      {isOpen && (
        <div className="lg:hidden mt-4 bg-background/60 backdrop-blur-3xl border border-foreground/10 rounded-3xl py-6 px-6 relative z-50 shadow-2xl overflow-hidden" dir="rtl">
          <div className="absolute top-0 left-0 w-32 h-32 bg-primary/10 blur-3xl rounded-full -ml-16 -mt-16 pointer-events-none" />
          <div className="absolute bottom-0 right-0 w-32 h-32 bg-secondary/10 blur-3xl rounded-full -mr-16 -mb-16 pointer-events-none" />
          ...
          <div className="flex flex-col gap-2 relative z-10">
            {navLinks.map((item) => (
              <div key={item.name}>
                <Link
                  href={item.href}
                  className="flex items-center justify-between text-lg text-foreground/80 hover:text-primary transition-colors py-3 px-4 rounded-2xl hover:bg-foreground/5 group"
                  style={{ fontWeight: 500, fontFamily: "'ThmanyahSans', system-ui" }}
                  onClick={(e) => {
                    e.preventDefault();
                    setIsOpen(false);
            ...
            <div className="flex items-center gap-3 pt-2 md:hidden">
              <Button
                className="flex-1 h-14 bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl shadow-lg active:scale-95 transition-all"
                style={{ fontWeight: 700, fontFamily: "'ThmanyahSans', system-ui" }}
                onClick={() => {
                  window.open(
                    downloadUrl || siteConfig.links.latestRelease,
